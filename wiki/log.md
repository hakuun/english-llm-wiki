...
批改 `[[study-plans/daily/2026-09-18]]`（status → completed）。**App 自动判分 8/11**。

> ⚠️ 批改时先误记为「11/15」，经核对 `app/index.html` 的判分逻辑后更正：App **只对单词填空判对错**（`isWordItem`），句子级填空（w1/w2/l2）显示「对照参考」不计分。自动判分的真实分母是 11 —— App 的 8/11 从一开始就是对的，是我一开始按 15 题算错了。
...
1. ⭐ **for/since 规则完全掌握！** — Part A 选择 4/4 (100%) ✅，Part C 改错正确 ✅（`since five years` → `for five years`），Part D 翻译满分 ✅（`I have been a software developer since 2020.`）。学习者清楚地知道：for + 时间段（period），since + 时间点（point）。这是今天最重要的成果——第二节现在完成时课的核心目标已达成。
